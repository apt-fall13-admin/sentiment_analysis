    ...
    for tweet in tweets:
        if 'geo' in tweet:
            if not(tweet['geo'] is None):
                ...
                #print lat,lon
                for key,value in state_co.items():
                    if lat>value[0] and lat<value[1] and lon>value[2] and lon<value[3]:
                        #print "State is " + key
                        state = key
                        sent_score = 0
                        if "text" in tweet:
                        ...
                        if state in happy_state:
                            happy_state[state]+=sent_score
                        else:
                            happy_state[state]=sent_score 
                    
    for w in sorted(happy_state, key=happy_state.get, reverse=True):
        to_print = states[w]
        count +=1
